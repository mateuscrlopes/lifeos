// LifeOS — Painel da Casa v3
// Aumenta a legibilidade e permite registrar cuidados de plantas no tablet.
(() => {
  'use strict';
  ...
  function assinatura(plantas) {
    return plantas
      .map(planta => `${planta.id}:${planta.rotinasPendentes.map(rotina => `${rotina.id}-${rotina.proxima_realizacao || ''}`).join(',')}`)
      .join('|');
  }
    ...
    return (data || [])
      .map(planta => ({
        ...
        rotinasPendentes: (planta.planta_rotinas || [])
          .filter(rotina => rotina.ativa && (!rotina.proxima_realizacao || rotina.proxima_realizacao <= hoje))
      ...
      .sort((a, b) => {
        ...
        return aData.localeCompare(bData) || nomePlanta(a).localeCompare(nomePlanta(b), 'pt-BR');
      });
  }
  ...
  function renderizarPlantas() {
    ...
    area.innerHTML = plantasPendentes.slice(0, 5).map(planta => {
      ...
      const meta = `${rotina.tipo || 'Cuidado pendente'}${restante ? ` · +${restante} cuidado${restante > 1 ? 's' : ''}` : ''}`;
    ...
    area.querySelectorAll('[data-pc-planta-id]').forEach(botao => {
      botao.addEventListener('click', () => abrirCuidados(botao.dataset.pcPlantaId));
    });
  ...
  function renderizarDestaque() {
    ...
    area.querySelector('.pc-cuidados-highlight')?.addEventListener('click', () => abrirCuidados());
  }

  async function sincronizar() {
    if (sincronizando || !contextoPronto()) return;
    sincronizando = true;
    try {
      const novas = await buscarPendencias();
      const novaAssinatura = assinatura(novas);
      plantasPendentes = novas;
      if (novaAssinatura !== assinaturaAtual || !document.querySelector('.pc-planta-action')) {
  ...
  function agendarSincronizacao() {
    ...
    agendamento = window.setTimeout(sincronizar, 180);
  }
  ...
  function abrirCuidados(plantaId = null) {
    fecharModal();
    const modal = document.createElement('div');
    modal.id = 'pcPlantModal';
    modal.className = 'pc-plant-modal';
    modal.dataset.plantaId = plantaId || '';
    modal.innerHTML = '<section class="pc-plant-dialog" role="dialog" aria-modal="true" aria-label="Cuidados das plantas"></section>';
    modal.addEventListener('click', evento => {
      if (evento.target === modal || evento.target.closest('[data-pc-close]')) fecharModal();
    });
  ...
  function renderizarModal(mensagem = '', erro = false) {
    ...
    const total = plantas.reduce((soma, planta) => soma + planta.rotinasPendentes.length, 0);
    ...
    dialogo.innerHTML = `
      <header class="pc-plant-header">
        <div>
          ...
          <div class="pc-plant-subtitle">${total ? `${total} ${total === 1 ? 'cuidado pendente' : 'cuidados pendentes'}` : 'Nenhum cuidado pendente agora'}</div>
        </div>
      ...
      <div class="pc-plant-content">
        ${total ? plantas.map(planta => `
          <section class="pc-care-plant">
            ${plantas.length > 1 ? `<div class="pc-care-plant-name">${escapar(nomePlanta(planta))}</div>` : ''}
            ${planta.rotinasPendentes.map(rotina => `
              <div class="pc-care-row">
                ...
                <button type="button" class="pc-care-button" data-pc-care data-planta-id="${escapar(planta.id)}" data-rotina-id="${escapar(rotina.id)}">Registrar cuidado</button>
              </div>`).join('')}
          </section>`).join('') : `
    ...
    dialogo.querySelectorAll('[data-pc-care]').forEach(botao => {
      botao.addEventListener('click', () => registrarCuidado(botao));
    });
  }
  ...
  async function registrarCuidado(botao) {
    const planta = plantasPendentes.find(item => String(item.id) === String(botao.dataset.plantaId));
    const rotina = planta?.rotinasPendentes.find(item => String(item.id) === String(botao.dataset.rotinaId));
    ...
    try {
      ...
      planta.rotinasPendentes = planta.rotinasPendentes.filter(item => String(item.id) !== String(rotina.id));
      plantasPendentes = plantasPendentes.filter(item => item.rotinasPendentes.length);
      ...
      window.setTimeout(sincronizar, 250);
    } catch (erro) {
  ...
  function interceptarContextoPlanta(evento) {
    const item = evento.target.closest('#painelContextoCasa [data-pagina="inicio"]');
    if (!item || !/cuidado de planta/i.test(item.textContent)) return;
  ...
  function iniciar() {
    carregarCss();
    document.addEventListener('click', interceptarContextoPlanta, true);

    const observar = () => {
      const inicio = document.getElementById('pag-inicio');
      if (!inicio) return false;
      new MutationObserver(agendarSincronizacao).observe(inicio, {
        childList: true,
        subtree: true,
        characterData: true,
      });
